...
export default function PaymentsCreate({ members, paymentTypes, selectedMemberId, memberPaidPeriods, memberPaidOneTime, currentYear }: Props) {
    ...
    const handlePaymentTypeChange = (typeId: string) => {
        const pt = paymentTypes.find((t) => String(t.id) === typeId);
        const now = new Date();
        let period = '';
        if (pt?.billing_cycle === 'monthly') {
            period = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
        } else if (pt?.billing_cycle === 'annual') {
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="mb-6 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-muted-foreground">Record a new payment from a member</p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Select the member and payment information</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select value={data.payment_type_id} onValueChange={handlePaymentTypeChange}>
                                                ...
                                                return (
                                                    <SelectItem key={pt.id} value={String(pt.id)} disabled={oneTimePaid}>
                                                        <span className="flex items-center gap-2">
                                                            {pt.name}
                                                            {pt.amount ? ` (₱${Number(pt.amount).toFixed(2)})` : ''}
                                                            {oneTimePaid && (
                                    ...
                                    {paymentTypes.length === 0 && (
                                        <p className="text-muted-foreground text-sm">
                                            ...
                                            <Link href="/payment-types/create" className="underline">
                                                Create one first.
                                            </Link>
                                        </p>
                                    )}
                                ...
                                <div className="space-y-2 md:col-span-2">
                                    ...
                                    <Textarea id="notes" value={data.notes} onChange={(e) => setData('notes', e.target.value)} placeholder="Optional remarks..." rows={2} />
                                </div>
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        {preview && (
                                            <div className="relative">
                                                <img src={preview} alt="Preview" className="h-20 w-20 rounded-md border object-cover" />
                                                <button
                                                    type="button"
                                                    onClick={removeFile}
